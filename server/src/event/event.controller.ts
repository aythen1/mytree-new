...
export class EventsController {
  ...
  async findByCreator(@Param('creatorId') creatorId: string) {
    try {
      ...
      return {
        error: 'No se pudieron recuperar los eventos creados por el usuario',
      };
    }
  }
  ...
  async findInvitedByStatus(
    ...
    @Query('aceptada') aceptada: boolean,
  ) {
    try {
      const invitedUsers = await this.eventsService.findInvitedByStatus(
        leida,
        aceptada,
      );
      return invitedUsers;
